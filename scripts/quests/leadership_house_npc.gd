...
var intro_dialogues: Array[Dictionary] = [
	{"name": "James", "text": "HI there! My name is James."},
	{"name": "James", "text": "I'm a leader from the past, and there is a serious issue I need your help with."},
	{"name": "James", "text": "My city is collapsing and there are citizens stuck inside."},
	{"name": "James", "text": "I need you to guide the citizens out of the city and save their lives"},
	{"name": "James", "text": "Will you accept this quest?"}
]
var robot_offer_dialogues: Array[Dictionary] = [
	{"name": "James", "text": "Thank you for rescuing my people. You did an incredible job out there."},
	{"name": "James", "text": "I have a second mission for you now."},
	{"name": "James", "text": "We need someone to lead our robot response operation before time runs out."},
	{"name": "James", "text": "Will you take on the robot leadership mission?"}
]
var post_robot_dialogues: Array[Dictionary] = [
	{"name": "James", "text": "Thank you again for everything you've done for our city."},
	{"name": "James", "text": "Both leadership missions are complete. The people are safe because of you."}
]
var active_dialogues: Array[Dictionary] = []
var dialogue_mode = "intro"
...
const QUEST_1_ID = "leadership_quest_1"
const QUEST_2_ID = "leadership_quest_2"
const QUEST_DOMAIN = "Leadership"
const QUEST_1_SCENE = "res://scenes/World.tscn"
const QUEST_2_SCENE = "res://scenes/World2.tscn"
...
func next_dialogue():
	...
	if current_dialogue_index < active_dialogues.size():
		...
		# Show choice only when this branch is offering a quest.
		if _dialogue_has_choice() and current_dialogue_index == active_dialogues.size():
			show_choice_ui()
		else:
			end_dialogue()

func _select_dialogue_set():
	var quest_1_done: bool = false
	var quest_2_done: bool = false
	if EndingManager:
		quest_1_done = EndingManager.is_quest_completed(QUEST_1_ID, QUEST_DOMAIN)
		quest_2_done = EndingManager.is_quest_completed(QUEST_2_ID, QUEST_DOMAIN)
	
	if quest_1_done:
		if quest_2_done:
			dialogue_mode = "post_robot_complete"
			active_dialogues = post_robot_dialogues
		else:
			dialogue_mode = "robot_offer"
			active_dialogues = robot_offer_dialogues
	else:
		dialogue_mode = "intro"
		active_dialogues = intro_dialogues

func _dialogue_has_choice() -> bool:
	return dialogue_mode == "intro" or dialogue_mode == "robot_offer"
...
func show_dialogue():
	...
	var current: Dictionary = active_dialogues[current_dialogue_index]
	...
	if current_dialogue_index == active_dialogues.size() - 1:
...
func handle_choice_selection():
	...
	if selected_choice == 0:  # Yes
		if dialogue_mode == "robot_offer":
			print("[NPC] Player accepted robot leadership quest! Loading World2...")
			get_tree().change_scene_to_file(QUEST_2_SCENE)
		else:
			print("[NPC] Player accepted first leadership quest! Loading World...")
			get_tree().change_scene_to_file(QUEST_1_SCENE)
	else:  # No

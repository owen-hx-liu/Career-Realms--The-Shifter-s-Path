...
var base_dialogues = [
	{"name": "Paya", "text": "Hi there! My name is Paya."},
	{"name": "Paya", "text": "I'm a Herbalist from the past, and there is a serious issue I need your help with."},
	{"name": "Paya", "text": "The village has recently faced a new disease that is sweeping through households!"},
	{"name": "Paya", "text": "I need you to diagnose and cure the villagers"},
	{"name": "Paya", "text": "Will you accept this quest?"}
]
var advanced_dialogues = [
	{"name": "Paya", "text": "You healed the first outbreak. Now we can attempt a risky bioengineering ritual."},
	{"name": "Paya", "text": "An alien plant sample is unstable and mutating each organ independently."},
	{"name": "Paya", "text": "We need to splice Root, Stem, and Canopy genomes before the lab collapses."},
	{"name": "Paya", "text": "Use the Lab Terminal tools wisely. The fungus will attack your synthesis links."},
	{"name": "Paya", "text": "Will you begin the Alien Gene-Splicing quest?"}
]
var dialogues = []
...
func start_dialogue():
	...
	_refresh_dialogues_for_progress()
	is_chatting = true
...
func show_dialogue():
	...
	var sprite = dialogue_box.get_node_or_null("Sprite2D")
	if sprite:
		sprite.position = Vector2(700, 355)
	...
	if name_label:
		...
		name_label.position = Vector2(580, 315)
	...
	if text_label:
		...
		text_label.position = Vector2(580,335)
...
func handle_choice_selection():
	...
	if selected_choice == 0:  # Yes
		if _should_start_advanced_medicine_quest():
			print("[NPC] Player accepted quest! Loading AlienBioengineeringQuest...")
			get_tree().change_scene_to_file("res://scenes/world_scenes/AlienBioengineeringQuest.tscn")
		else:
			print("[NPC] Player accepted quest! Loading Forest Quest...")
			get_tree().change_scene_to_file("res://scenes/world_scenes/forest.tscn")
	else:  # No
		print("[NPC] Player declined quest")
		end_dialogue()

func _should_start_advanced_medicine_quest() -> bool:
	if EndingManager == null:
		return false
	return EndingManager.is_quest_completed("forest_quest", "Medicine")

func _refresh_dialogues_for_progress() -> void:
	if _should_start_advanced_medicine_quest():
		dialogues = advanced_dialogues.duplicate(true)
	else:
		dialogues = base_dialogues.duplicate(true)

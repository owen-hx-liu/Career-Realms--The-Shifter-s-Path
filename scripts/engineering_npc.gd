...
var dialogues = [
	{"name": "Steve", "text": "Hello traveler, my name is Steve."},
	{"name": "Steve", "text": "I'm an engineer, but there's a serious issue right now."},
	{"name": "Steve", "text": "The ancient craft of canal building is collapsing."},
	{"name": "Steve", "text": "To fix this, I need someone to restore the canals of Ancient Egypt."},
	{"name": "Steve", "text": "Will you accept this quest?"}
]

# Which quest the current offer launches (chosen by _setup_offer based on progress)
var current_quest_scene := "res://scenes/maps/AncientEgyptMap.tscn"
...
# Pick which engineering quest Steve offers, based on the player's progress.
# Ancient Egypt first; once it's restored he pitches the Dyson Swarm; after both
# are done he invites the player back to improve their swarm score.
func _setup_offer():
	var em = get_node_or_null("/root/EndingManager")
	var egypt_done = em and em.has_method("is_quest_completed") and em.is_quest_completed("flood_egypt_1", "Engineering")
	var dyson_done = em and em.has_method("is_quest_completed") and em.is_quest_completed("engineering_quest_2", "Engineering")

	if not egypt_done:
		current_quest_scene = "res://scenes/maps/AncientEgyptMap.tscn"
		dialogues = [
			{"name": "Steve", "text": "Hello traveler, my name is Steve."},
			{"name": "Steve", "text": "I'm an engineer, but there's a serious issue right now."},
			{"name": "Steve", "text": "The ancient craft of canal building is collapsing."},
		...
		current_quest_scene = "res://scenes/dyson_swarm.tscn"
		dialogues = [
			{"name": "Steve", "text": "The canals flow once more — fine work, Shifter."},
			{"name": "Steve", "text": "But a far stranger plea just reached us... from the stars."},
			{"name": "Steve", "text": "A distant colony's sun is dying. Their world is going dark."},
			{"name": "Steve", "text": "They need a Dyson Swarm — a fleet of mirrors to harvest a star's light."},
			{"name": "Steve", "text": "It's the hardest engineering I know. Will you take it on?"}
		]
	else:
		current_quest_scene = "res://scenes/dyson_swarm.tscn"
		dialogues = [
			{"name": "Steve", "text": "Both realms of engineering stand restored — remarkable."},
			{"name": "Steve", "text": "The star-colony still speaks of you. Care to return and perfect the swarm?"}
		]
...
func show_dialogue():
	...
	var sprite = dialogue_box.get_node_or_null("Sprite2D")
	if sprite:
		sprite.position = Vector2(288, 365)
	...
	if name_label:
		...
		name_label.position = Vector2(165, 325)
	...
	if text_label:
		...
		text_label.position = Vector2(165, 345)
...
func handle_choice_selection():
	...
	if selected_choice == 0:  # Yes
		print("[NPC] Player accepted quest! Loading: ", current_quest_scene)
		get_tree().change_scene_to_file(current_quest_scene)
	else:  # No

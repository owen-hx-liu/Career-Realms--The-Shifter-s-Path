...
@export var spawn_position_in_mainhub: Vector2 = Vector2(454.3324, 558.3327)
@export var spawn_position_in_starcontainerroom1: Vector2 = Vector2(169,196)

func _ready():
	# Set the current scene when the house loads
	global.current_scene = "StarHouse"
...
func _on_area_2d_body_entered2(body: Node2D) -> void:
	...
	if body.name.to_lower() == "player" or body.has_method("player"):
		...
		# Set where the player should spawn in MainHub
		global.player_spawn_position = spawn_position_in_starcontainerroom1
...
func change_scene():
	if global.transition_scene == true:
		...
		if global.current_scene == "StarHouse":
	...
	if global.transition_scene2 == true:
		...
		if global.current_scene == "StarHouse":
			...
			get_tree().change_scene_to_file("res://scenes/maps/starcontainerroom.tscn")
			global.finish_changescenes()

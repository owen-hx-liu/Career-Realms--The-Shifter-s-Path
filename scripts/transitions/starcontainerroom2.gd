...
@export var spawn_position_in_scr: Vector2 = Vector2(317.7897, 34.19244)
@export var spawn_position_in_scr3: Vector2 = Vector2(29.59566, 45.03083)

var transition_cooldown: float = 1.0  # 1 second cooldown

func _ready():
	global.current_scene = "Starcontainerroom2"
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	if body.name.to_lower() == "player" or body.has_method("player"):
		if global.scene_transition_cooldown <= 0:  # Check global cooldown
			...
			global.player_spawn_position = spawn_position_in_scr
		else:
